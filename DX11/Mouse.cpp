...
private:
	...
	friend void CMouse::ProcessMessage(UINT message, WPARAM wParam, LPARAM lParam);
...
CMouse::Impl* CMouse::Impl::s_mouse = nullptr;

void CMouse::SetWindow(HWND window)
{
...
// Public constructor.
CMouse::CMouse() noexcept(false)
	: pImpl(std::make_unique<Impl>(this))
{
}

// Move constructor.
CMouse::CMouse(CMouse&& moveFrom) noexcept
...
// Move assignment.
CMouse& CMouse::operator= (CMouse&& moveFrom) noexcept
{
	pImpl = std::move(moveFrom.pImpl);
	pImpl->mOwner = this;
	return *this;
...
CMouse::State CMouse::GetState() const
{
	State state;
...
void CMouse::SetMode(Mode mode)
{
	pImpl->SetMode(mode);
}

bool CMouse::IsConnected() const
{
	return pImpl->IsConnected();
}

bool CMouse::IsVisible() const
{
	return pImpl->IsVisible();
}

void CMouse::SetVisible(bool visible)
{
...
void CMouse::ButtonStateTracker::Update(const CMouse::State& state)
{
